...
const FilterProducer = (listNameUsed) => {
    ...
    <div className='filter-listing'>
      ...
      <div className='btn-wrap'>
        <div className='demo-inline-spacing'>
          <Link to={`/manager/producer/add-producer`}>
            <Button color='primary'>
              Thêm
            </Button>
          </Link>
          <Offcanvas className='wrap-form w-50' direction={canvasPlacement} isOpen={canvasOpen} toggle={toggleCanvasStart}>
            <OffcanvasHeader toggle={toggleCanvasStart}>Thêm nhà cung cấp</OffcanvasHeader>
            <OffcanvasBody
              className={classnames({
                'my-auto mx-0 flex-grow-0': canvasPlacement === 'start' || canvasPlacement === 'end'
              })}
            >
              <FormChairCategory parentCallback={callbackCanvasOpen} listNameUsed={listNameUsed} />
            </OffcanvasBody>
          </Offcanvas>
        </div>
      </div>
