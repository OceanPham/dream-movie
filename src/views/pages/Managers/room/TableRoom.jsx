...
import { useDeleteRoom, useGetALLRoom } from './hook'
...
const columnHeaders = ["STT", "Tên ghế", "Sức Chứa", "Rạp Chiếu", "Cập Nhật Lần Cuối", "Hành Động"]
...
const TableRoom = () => {
  ...
  const { status, data: dataListRoom } = useGetALLRoom()
  const { status: sttDelete, mutate: deleteRoom } = useDeleteRoom()


  const listNameUsed = dataListRoom && dataListRoom?.length > 0 && dataListRoom.map(((item) => item?.name?.toLowerCase()?.trim()))
  ...
  return (
    <>
      <Card className='wrap-container border-none'>
        <FilterRoom listNameUsed={listNameUsed} />
        <Table responsive>
          {status === 'loading' ? (
            <React.Fragment>
            ...
            <React.Fragment>
              ...
              <tbody>
                ...
                {dataListRoom?.length > 0 ? dataListRoom.sort((a, b) => b.id - a.id).slice(startIndex, endIndex).map((item, index) => {
                  return (
                    <tr>
                      <td className='ps-3'>
                        {index + 1 + startIndex}
                      ...
                      <td className='string-name'>
                        {item?.capacity ? item?.capacity : <> <NoData /> </>}
                      </td>

                      <td className='string-name'>
                        {item?.cinema ? item?.cinema?.name : <> <NoData /> </>}
                      </td>
                      ...
                      <td className='fw-bold string-name d-flex justify-content-center'>
                        <div className='column-action d-flex align-items-center'>
                          ...
                          <Link to={`/manager/Room`} id={`pw-tooltip-${item.id}`}>
                ...
                <Offcanvas className='wrap-form' direction={canvasPlacement} isOpen={canvasOpen} toggle={toggleCanvasStart}>
                  ...
                  <OffcanvasBody className={classNames({
                    ...
                    <FormRoom parentCallback={callbackCanvasOpen} />
                  </OffcanvasBody>
                </Offcanvas>
              </tbody>
            </React.Fragment>
